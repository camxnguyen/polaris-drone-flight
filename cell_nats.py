# Code taken from OATS center at Purdue
#!/usr/bin/env python3
...
#found, assume the modem is connected to a 'Umts' network.
    ...
    if modem_data[modem_path][modem_signal_iface].get("Lte"):
...
#ID has changed and needs to have the signal info update rate reset.
    ...
    try:
        ...
        for item, value in signal_dict.items():
            # Truncate all floats to 1 decimal point
            signal[str(item)] = float(f"{value:.1f}")
